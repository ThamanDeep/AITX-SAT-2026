...
"""
import json, os, sys
from _util import now_iso, atomic_write, read_json, autoresearch_home


def _registry_path():
    return os.path.join(autoresearch_home(), "registry.json")
...
def register(rid, uid, plat, cid, goal, jid, wid=None):
    reg = _read_registry()
    run_dir = os.path.join(autoresearch_home(), "runs", rid)
    reg["runs"][rid] = {
