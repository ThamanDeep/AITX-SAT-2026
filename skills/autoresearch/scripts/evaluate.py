...
Usage:
    ...
    python evaluate.py score-policy '<cand_json>' '<champ_json>' [--min-cases N]
...
def score_policy(cand, champ, min_cases=1, acc_eps=0.005, lat_slack=1.3):
    """Pareto gate for the GPU-deal policy loop (AITX).

    Keep only if accuracy rises (by >= acc_eps), deal_safety does not drop,
    and retrieval latency stays within lat_slack × champion.
    """
    reasons = []
    if cand.get("n", 0) < min_cases:
        reasons.append(f"too few rollouts ({cand.get('n', 0)} < {min_cases})")
    if cand.get("accuracy", 0) < champ.get("accuracy", 0) + acc_eps:
        reasons.append(
            f"accuracy {cand.get('accuracy')} < champ {champ.get('accuracy')} + {acc_eps}"
        )
    if cand.get("deal_safety", 0) < champ.get("deal_safety", 0):
        reasons.append(
            f"deal_safety regression {cand.get('deal_safety')} < {champ.get('deal_safety')}"
        )
    champ_lat = champ.get("retrieval_s") or 1.0
    if cand.get("retrieval_s", 999) > champ_lat * lat_slack:
        reasons.append(
            f"retrieval {cand.get('retrieval_s')}s > {champ_lat * lat_slack:.2f}s slack"
        )
    decision = "MERGE" if not reasons else "REVERT"
    return {
        "decision": decision,
        "reason": "Pareto improve" if decision == "MERGE" else "; ".join(reasons),
        "candidate": cand,
        "champion": champ,
        "stability": round(
            cand.get("accuracy", 0) - champ.get("accuracy", 0), 4
        ),
    }
...
if __name__ == "__main__":
    ...
    elif cmd == "score-policy":
        # score-policy '<cand_json>' '<champ_json>' [--min-cases N]
        min_cases = int(args[args.index("--min-cases")+1]) if "--min-cases" in args else 1
        print(json.dumps(score_policy(json.loads(args[1]), json.loads(args[2]), min_cases=min_cases), indent=2))
    elif cmd == "log-result":
        scores_str = None
