...
- **SCOPE**: {{scope}}
- **DEPTH**: {{depth}}
- **MODE**: {{mode}}   # policy (GPU lessons) or knowledge (research.md)
...
## AITX POLICY MODE NOTES
When mode=policy, the target file is `champion-lessons.md`. Evaluate with:
```bash
python {{scripts_dir}}/evaluate.py score-policy '<cand_metrics_json>' '<champ_metrics_json>' --min-cases N
```
Pareto gate: accuracy must rise, deal_safety must not drop, retrieval_s ≤ 1.3× champion.
Use `scripts/auto_research_loop.py` for the headless continuous policy loop; this
cron template is for agent-driven knowledge research or one-shot policy runs.
